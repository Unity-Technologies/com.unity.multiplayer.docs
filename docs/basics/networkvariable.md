...
<a name="AnticipatedNetworkVariable"></a>
...
- **Snap Reconciliation:** 
  ...
  - *Example: Player presses a UI button and the UI immediately updates. If the server rejects the action, the UI reverts to its previous state.*
  - *Example: Player equips a new piece of armor and the player's model immediately updates. If the server rejects the action, the model reverts to the previous equipment.*

  Snap Reconciliation is enabled by creating the variable with`StaleDataHandling.Ignore` and providing no `OnReanticipate` callback, making it the default mode. In this mode, when you update the variable by calling `variable.Anticipate(val)`, the variable's display value is updated to the new value, and the variable will then track the next round trip to the server, ignoring any updates that arrive before one round trip is completed (updates that are considered "stale"). The general assumption is that operating in this mode will be accompanied by a message to the server (most likely an RPC) that will update the value on the server-side, and the anticipated value on the client will be left unchanged until that RPC has been processed by the server. This is done deterministically (which is to say, the anticipation system sends a message to the server and waits for a response to that specific message, as opposed to guessing based on current round trip time values).

- **Smooth Reconciliation:** 

  - *Example: The player is playing a fishing minigame. The distance from the fish to the player is represented by a float NetworkVariable. When the player presses a button, the fish jumps closer to the player. If the server simulation has a different result, the fish smoothly swims to the new location.*
  - *Example: The player is watching another player play the fishing minigame. The player is pressing the button faster than the network tick rate. In order to keep the fish from teleporting, the other client uses smooth reconciliation to smooth out the movement of the fish.*

  Smooth reconciliation is similar to the above, but adds an `OnReconcile` callback that enables it to make the correction less jarring, so that instead of snapping to the new value, it is smoothed out in some way. This is generally done via interpolation, but it can be done in any way you like. There are two ways you can handle this: If your smoothing function can be expressed as a function that takes a start value, an end value, and a percentage value between the two, you can use `variable.Smooth(start, end, duration, method)` to have the variable automatically handle interpolation for you. The classic example is smoothing a float value using `variable.Smooth(anticipatedValue, authoritativeValue, 0.25f, Mathf.Lerp)` - this will interpolate between `anticipatedValue` (the value you last provided to `variable.Anticipate`) and `authoritativeValue` (the value that just arrived from the server) over 0.25 seconds. The other way, if your needs are more complex, is to have some external process (say, in the `NetworkBehaviour`'s `Update` event) perform the smoothing by making new calls to `variable.Anticipate()` on each frame, providing new values until it reaches the point where the anticipated and authoritative values are the same.

  - **Server/Host Smoothing:** 

    - *Example: The host player is watching another player play the fishing minigame. The player is pressing the button regularly, but due to network latency and jitter, the inputs are not arriving smoothly on the host, resulting in jerky movement of the fish. When new inputs arrive, the host uses smoothing to hide the unevenness of the inputs.*
    
    Smoothing can also be performed on the server/host side. To do this, when you receive an RPC from the user to change a value, you can call `variable.Smooth()` from inside the RPC to smooth the visual display of the variable's value for host-client players.
    
    There are two important things to keep in mind with server/host smoothing: First is that, unlike on the client side, the `OnReanticipate` callback will never be called on the server. Second is that normally updates to the value on the server side affect both `Value` and `AuthoritativeValue`, a call to `Smooth` will affect the return value of the `Value` property, but **not** the return value of the `AuthoritativeValue` property, allowing you to differentiate between the actual value (`AuthoritativeValue`) and the visual value (`Value`) that you can use for rendering the effects of the variable.

- **Extrapolation:**

  * *Example: A progress bar shows the time remaining to defuse a bomb. To hide latency, the progress bar starts updating the moment the player interacts with the bomb, ticks up at the same rate the server does, and finishes when it expects the server to finish. The server regularly sends updates on the state of the progress bar, so if the client and server get out of sync, the client smoothly adjusts its displayed progress bar.*

  Extrapolation is a process wherein a server-controlled network variable is updated continuously to compensate for latency between the server and the client. In extrapolation, you calculate the **half**-round-trip time from the server to the client and simulate that amount of time forward for the variable, calling the same functions the server would call to calculate a new value for the variable. Generally, because server updates are not consistent in their timing, you will then also want to smooth the result as mentioned above. One thing to keep in mind, however, is that smoothing adds additional latency (as it takes that much additional time to reach the actual server value), so you will want to factor the amount of time you use for your smoothing operation into your simulation - making it half-round-trip plus smooth time.
...
- **Local Simulation and Reanticipation:**

  - *Example: The player is playing a fishing minigame. The distance from the player on two axes is represented by a Vector2 network variable. The fish swims side to side in a predictable way. If the player moves the joystick in the correct direction, the fish smoothly moves closer, otherwise it smoothly moves away. This is simulated on the client and the result is anticipated on the networkvariable. The input is sent to the server, which runs the same simulation and sends the results to the client. When the client gets results, they are in the past because of latency, so the client reapplies previous inputs to calculate a new anticipated value for this NetworkVariable. If the new value is different from the previous anticipated value, the client smoothly moves the fish to the new location.*

  This is more common for NetworkTransform than it is for NetworkVariable, but is still achievable for NetworkVariable as well when it is needed. This mode is similar to extrapolation, except that instead of updating the variable when you get an update from the server, you apply updates locally in response to player input (making this variable client-controlled) and ALSO constantly respond to updates from the server.
