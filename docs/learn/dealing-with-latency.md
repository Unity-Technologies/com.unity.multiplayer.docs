...
### Client Side Reinterpretation (state solving)
For a few reasons, clients can't always stay in sync tick by tick, frame by frame. It's possible to adapt your gameplay to allow your clients to be desynced for a few seconds, as long as they sync back again eventually.
This is usually the case for death ragdolls in games where a dead character's body doesn't influence your gameplay anymore and so can be desynced from one client the another. The server will sync to all clients "this character is now dead" and will send it's death position. From then on, clients can apply all the ragdoll physics they want with no need of syncing it between clients (as it has no gameplay impact, we don't care if I see the ragdoll in a tree and you see the ragdoll in a ditch).
Another example for this could be with assassination mechanics with client driven gameplay. Alice the assassin chases Bob the victim. Bob enters a room and close the door real quick behind them at time t=0 seconds. Also at t=0, Alice hits the "kill" button. Both actions being client driven and both actions reaching the server at the same time, the server would tell all clients "Alice killed Bob". But since locally Bob already had their door closed before receiving the kill message, the door would also be closed when executing this animation. A purist way of doing this would be to use server authoritative gameplay with prediction and correction on both clients. This does add complexity to your game. Another way could be with reinterpretation, where you would  use grates or have windows in your doors and reinterpret the kill action depending on local state. Alice could see a knife kill while the grate is still open (hasn't received the update from the server yet) and Bob could see a gun kill through the grate.
