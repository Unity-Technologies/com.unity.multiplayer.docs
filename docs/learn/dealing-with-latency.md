...
An elegant way to handle that desync could be with reinterpretation, where you could add windows in your doors and reinterpret the kill action depending on local client state. Alice could see a knife kill while the door is still open (hasn't received the "door closed" update from the server yet) and Bob could see a gun kill through the locally closed door through the window instead. In the end, clients only care about the end "Bob is dead and killed by Alice" state. You let clients reinterpret how they reach that final state according to their own different local context.
...
Some believe all clients and servers need to be in the exact same state as much as possible. This is true for high precision tactical shooters for example, where they'll go to great lengths making sure the game is fair for both attackers and targets. Outside of those cases, client reinterpretation can be a good way to keep immersion with your players when dealing with lag. As long as you end up in the same state and don't have a snowball effect of discrepancies accumulating, your players will be fooled. The goal is to give the "illusion" to your players are playing in the same world, when they actually are not.
