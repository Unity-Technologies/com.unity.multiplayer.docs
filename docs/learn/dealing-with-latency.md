...
### Controlled Desyncs
Corrections from mispredictions can be hard to handle elegantly and can take your players out of their immersion. Clients also can't always stay in sync tick by tick, frame by frame. It's possible to adapt your gameplay and animations to allow your clients to be desynced for a few seconds, as long as they come back in sync again eventually. This technique allows clients to resolve and reinterpret how they'll reach a common state independently, according to their own local context.
You can see this technique as a "smart context aware interpolation" or an "elegant correction" when predicting. It reuses some of Action Anticipation's technique where you use non-gameplay-important animations to hide discrepancies caused by lag and reinterpret how the client will reach the target authoritative state.

For example you could have an assassination mechanic where you don't want to allow corrections taking your players out of their immersion. You could have your client tell the server "I killed that target", have the server do some basic validation to prevent cheating like we've seen with server rewind, but still give advantage to the attacker. In this example, Alice the assassin would chase Bob the victim. Bob enters a room and closes the door real quick behind them at time t=0 seconds. Also at t=0, Alice hits the "kill" button. Both actions being client driven and both actions reaching the server at the same time, the server would tell all clients "Alice killed Bob" at t=0.1. Bob receives the "you got killed" message at time t=0.2. But since locally Bob already had their door closed at t=0, this would result in Alice appearing to kill Bob through the door.
A common solution to solve this would be to use server authoritative gameplay with prediction and correction on both clients like described in other sections on this page. The door would be corrected, it'd teleport back open and the killing animations would play as usual. That correction would remove your players from their immersion. Not great.
Another way could be to use action anticipation on the door closing and have the door be server driven. This way you'd remove the correction and desync problems altogether. It would still add lag to each door opening and closing though if you want your action to be instant with no animation to hide lag.
An elegant way to handle that desync could be with this controlled desyncs technique, where you could add windows in your doors and reinterpret the kill action depending on local client state. Alice could see a knife kill while the door is still open (hasn't received the "door closed" update from the server yet) and Bob could see a gun kill through the locally closed door through the window instead. In the end, clients only care about the end "Bob is dead and killed by Alice" state. You let clients reinterpret how they reach that final state according to their own different local context.
...
- death ragdolls in games where a dead character's body doesn't influence your gameplay anymore and so can be desynced from one client to another. The server will sync to all clients "this character is now dead" and will send it's death position. From then on, clients can apply all the ragdoll physics they want with no need of syncing it between clients (as it has no gameplay impact, we don't care if I see the ragdoll in a tree and you see the ragdoll in a ditch). Eventually the players will respawn or the ragdoll could disappear and your game state will be in sync again.

In each of those cases, you need to make sure the reinterpretation doesn't affect gameplay critical systems. If in the first example, the number of gun kills and knife kills are important to your player's stats, this solution might not be doable.

Some believe clients and servers need to be in the exact same state as much as possible, all the time. This is true for high precision tactical shooters for example, where they'll go to great lengths making sure the game is fair for both attackers and targets. Outside of those cases, Controlled Desyncs can be a good way to keep immersion with your players when dealing with lag. As long as you end up in the same state and don't have a snowball effect of discrepancies accumulating, your players will be fooled. The goal is to give to your players the "illusion" they are playing in the same world, when they actually are not.
