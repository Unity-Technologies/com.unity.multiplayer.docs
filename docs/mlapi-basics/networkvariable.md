...
### Permissions
By default `NetworkVariable` and it's subclasses can only be written to by the server (`NetworkVariable`ermission.ServerOnly). To change that set the permission to the desired value during initialization:
