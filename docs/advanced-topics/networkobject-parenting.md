...
## Hybrid Parenting
While you are required to have a NetworkObject on any GameObject you wish to parent another `NetworkObject` under, there is a hybrid approach that you can use as well.  
Let's take a look at the invalid example once more:
```
Sun
Tree
Camera
...
Vehicle (GameObject->NetworkObject)
  ...
  └─ Seat2 (GameObject)
```
If we were to make some adjustments to the player where the player's root model hierarchy was a nested `GameObject` under the root player's `GameObject` with a `NetworkObject` component like this:
```
Sun
Tree
Camera
Player (GameObject->NetworkObject)
└─PlayerModel-Root (GameObject)
  ├─ Head (GameObject)
  ├─ Body (GameObject)
  ├─ Arms (GameObject)
  │  ├─ LeftArm (GameObject)
...
Vehicle (GameObject->NetworkObject)
  ...
  └─ Seat2 (GameObject)
```
Then we could create a special `NetworkBehaviour` that leverages from the virtual method `NetworkBehaviour.OnNetworkObjectParentChanged` to know when the player is being parented.  First, we would need a way to get an available seat on the vehicle like in the below (very) simple example:
```csharp
public class VehicleBehaviour : NetworkBehaviour
{
    public List<GameObject> Seats; // Preset in editor
...
```
The simplest and least modular way is to manually add the seat GameObjects to the `VehicleBehaviour.Seats` property within the editor and parse through that list checking to see if any of the seats have children.  If so, then it moves on to the next .  If not, then it returns the empty seat `GameObject`.

However, we now need to handle the actual parenting.

[Download The Exported Custom Parenting Example Here](./CustomParentingExample.unitypackage)
